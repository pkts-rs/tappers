...
struct WintunApi {
    ...
    WintunCreateAdapter: unsafe extern "C" fn(name: PCWSTR, tunnel_type: PCWSTR, requested_guid: GUID) -> *mut WintunAdapter,
    ...
    /// two.
    WintunStartSession: unsafe extern "C" fn(adapter: *mut WintunAdapter, capacity: u32) -> *mut WintunSession,
    ...
    WintunReceivePacket: unsafe extern "C" fn(session: *mut WintunSession, packet_size: *mut u32) -> *mut u8,
    ...
    WintunAllocateSendPacket: unsafe extern "C" fn(session: *mut WintunSession, packet_size: u32) -> *mut u8,
...
impl Wintun {
    ...
    unsafe fn resolve_func(lib: *mut libc::c_void, func_name: &[u8]) -> io::Result<*const libc::c_void> {
        GetProcAddress(lib, func_name.as_ptr()).ok_or(io::Error::last_os_error()).map(|f| f as *const libc::c_void)
    }
    ...
    fn resolve_api(lib: *mut libc::c_void) -> io::Result<WintunApi> {
        unsafe {
            Ok(WintunApi {
                ...
                WintunGetAdapterLUID: mem::transmute(Self::resolve_func(lib, b"WintunGetAdapterLUID\0")?),
                WintunGetRunningDriverVersion: mem::transmute(Self::resolve_func(lib, b"WintunGetRunningDriverVersion\0")?),
                ...
                WintunGetReadWaitEvent: mem::transmute(Self::resolve_func(lib, b"WintunGetReadWaitEvent\0")?),
                WintunReceivePacket: mem::transmute(Self::resolve_func(lib, b"WintunReceivePacket\0")?),
                WintunReleaseReceivePacket: mem::transmute(Self::resolve_func(lib, b"WintunReleaseReceivePacket\0")?),
                WintunAllocateSendPacket: mem::transmute(Self::resolve_func(lib, b"WintunAllocateSendPacket\0")?),
    ...
    pub fn new() -> io::Result<Self> {
        unsafe {
            ...
            let api = Self::resolve_api(lib).map_err(|e| io::Error::new(io::ErrorKind::NotFound, e))?;
    ...
    pub fn create_adapter(&self, name: &[u16], tunnel_type: &[u16], requested_guid: GUID) -> io::Result<NonNull<WintunAdapter>> {
        if name.last() != Some(&0) {
            return Err(io::Error::new(io::ErrorKind::InvalidInput, "adapter name not null-terminated"))
        }

        if tunnel_type.last() != Some(&0) {
            return Err(io::Error::new(io::ErrorKind::InvalidInput, "tunnel type not null-terminated"))
        }

        let handle = unsafe { (self.api.WintunCreateAdapter)(name.as_ptr(), tunnel_type.as_ptr(), requested_guid) };
    ...
    pub fn start_session(&self, adapter: &mut WintunAdapter, capacity: u32) -> io::Result<NonNull<WintunSession>> {
    ...
    pub fn recv_packet(&self, session: &mut WintunSession, packet_size: &mut u32) -> io::Result<WintunPacket> {
    ...
    pub fn allocate_packet(&self, session: &mut WintunSession, packet_size: u32) -> io::Result<WintunPacket> {
